...
def _normalize_px(px: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
    """Swap inverted coordinates so PIL never raises during debug rendering."""
    x0, y0, x1, y1 = px
    if x0 > x1:
        x0, x1 = x1, x0
    if y0 > y1:
...
def draw_debug_page(
    ...
    for bbox, role, label in annotations:
        ...
        px = _normalize_px(pdf_bbox_to_pixel(bbox, dpi))
...
def draw_debug_question(
    ...
    img = Image.open(page_image_path).convert("RGB")
    qpx = _normalize_px(pdf_bbox_to_pixel(question_bbox, dpi))
    qpx = (
        max(0, qpx[0] - pad),
        max(0, qpx[1] - pad),
    ...
    if qpx[2] - qpx[0] < 4 or qpx[3] - qpx[1] < 4:
        return
    crop = img.crop(qpx)
    ...
    for bbox, role, label in annotations:
        ...
        if rel[2] - rel[0] < 1 or rel[3] - rel[1] < 1:
            continue
        try:
