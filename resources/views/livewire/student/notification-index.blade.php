...
<div style="max-width: 920px;">
    ...
    <div class="space-y-2">
        @forelse ($this->items as $n)
            @php [$icon, $color, $tag] = $meta[$n->type] ?? ['bell', 'var(--text-soft)', 'Update']; $d = $n->data; @endphp
            <div class="flex" style="gap: 14px; background: var(--surface); border: 1px solid var(--border); border-radius: var(--r-lg); padding: 15px 18px; {{ $n->read_at ? '' : 'border-left: 3px solid '.$color.'; background: rgba(var(--accent-rgb),0.04);' }}">
                ...
                <div class="flex items-center" style="gap: 8px; flex: none;">
                    @if (! empty($d['url']))
                        <a href="{{ $d['url'] }}" wire:navigate class="btn btn-ghost btn-sm"><x-icon name="play" size="12"/> Open</a>
                    @endif
                    @unless ($n->read_at)
