...
<div class="flex items-center justify-center" style="min-height: 100vh; background: var(--ivory-deep);">
    <div style="width: 100%; max-width: 420px; padding: 40px;">
        ...
        <div style="padding: 6px 12px; background: rgba(212,164,55,0.1); border: 1px solid rgba(212,164,55,0.3); border-radius: 6px; font-size: 12px; font-weight: 600; color: var(--gold-700); text-transform: uppercase; letter-spacing: 0.08em; display: inline-flex; margin-bottom: 16px;">
            Action Required
        </div>
        ...
        <p style="color: var(--text-soft); margin-top: 8px; font-size: 14px; line-height: 1.5;">
            You must set a new password before continuing. Choose something strong.
        </p>
        ...
        <form method="POST" action="{{ route('v2.super_admin.change_password.post') }}" style="margin-top: 28px;">
            @csrf
            ...
            <div style="margin-top: 16px;">
                ...
                <input class="input" type="password" name="password_confirmation" required autocomplete="new-password"
                       placeholder="Repeat password" style="margin-top: 6px;"/>
            </div>
