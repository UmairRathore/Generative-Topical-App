@php
    $user = auth('v2_teacher')->user();
    $currentRouteName = request()->route()?->getName() ?? '';

    $nav = [
        ['section' => 'Teach'],
        ['id' => 'v2.teacher.dashboard', 'icon' => 'home', 'label' => 'Dashboard', 'href' => route('v2.teacher.dashboard')],
        ['id' => 'v2.teacher.exams', 'icon' => 'clipboard', 'label' => 'Exams', 'href' => route('v2.teacher.exams.index')],
        ['id' => 'v2.teacher.classes', 'icon' => 'users', 'label' => 'Classes', 'href' => route('v2.teacher.classes.index')],
        ['id' => 'v2.teacher.stats', 'icon' => 'chart', 'label' => 'Class Analytics', 'href' => route('v2.teacher.stats')],
    ];
@endphp
<!DOCTYPE html>
...
<div class="flex min-h-screen" style="background: var(--bg);">
    ...
    <aside class="flex flex-col"
        ...
        <div style="padding: 20px 18px 16px; border-bottom: 1px solid rgba(var(--accent-rgb),0.15);">
            <a href="{{ route('v2.teacher.dashboard') }}">
        ...
        <div style="padding: 12px 16px 4px;">
            <div class="inline-flex items-center gap-1.5"
                ...
                <x-icon name="edit" size="11" stroke="2" />
                Teacher
            </div>
        </div>
        ...
        <div style="padding: 14px; border-top: 1px solid rgba(var(--accent-rgb),0.15);">
            <div class="flex items-center gap-2.5" style="padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.04);">
                ...
                <div style="min-width: 0; flex: 1;">
                    <div class="truncate" style="font-size: 13px; font-weight: 600;">{{ $user?->name ?? 'Teacher' }}</div>
                    <div class="truncate" style="font-size: 11px; opacity: 0.65;">{{ $user?->school?->name ?? 'School' }} · Teacher</div>
                </div>
                <form method="POST" action="{{ route('v2.teacher.logout') }}">
                    @csrf
