@php
    $user = auth('v2_school_admin')->user();
    ...
    $nav = [
        ...
        ['id' => 'v2.school.subjects',  'icon' => 'book-open',    'label' => 'Subjects',   'href' => route('v2.school.subjects.index')],
        ['id' => 'v2.school.classes',   'icon' => 'calendar',     'label' => 'Classes',    'href' => route('v2.school.classes.index')],
...
<div class="flex min-h-screen" style="background: var(--bg);">
    ...
    <aside class="flex flex-col"
        ...
        <div style="padding: 20px 18px 16px; border-bottom: 1px solid rgba(212,164,55,0.15);">
            <a href="{{ route('v2.school.dashboard') }}">
        ...
        <div style="padding: 14px; border-top: 1px solid rgba(212,164,55,0.15);">
            <div class="flex items-center gap-2.5" style="padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.04);">
                ...
                <div style="min-width: 0; flex: 1;">
                    <div class="truncate" style="font-size: 13px; font-weight: 600;">{{ $user?->name ?? 'School Admin' }}</div>
                    <div class="truncate" style="font-size: 11px; opacity: 0.65;">{{ $user?->school?->name ?? 'School' }} · Admin</div>
                </div>
                <form method="POST" action="{{ route('v2.school.logout') }}" style="flex: none;">
                    @csrf
