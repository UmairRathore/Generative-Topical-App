@php
    $user = auth('v2_branch_admin')->user();
    $currentRouteName = request()->route()?->getName() ?? '';

    $nav = [
        ['section' => 'Branch'],
        ['id' => 'v2.branch.index',    'icon' => 'home',   'label' => 'Dashboard', 'href' => route('v2.branch.index')],
        ['section' => 'Analytics'],
...
<div class="flex min-h-screen" style="background: var(--bg);">
    ...
    <aside class="flex flex-col"
        ...
        <div style="padding: 12px 16px 4px;">
            <div class="inline-flex items-center gap-1.5"
                ...
                <x-icon name="school" size="11" stroke="2" />
                Branch Admin
            </div>
        </div>
        ...
        <div style="padding: 14px; border-top: 1px solid rgba(var(--accent-rgb),0.15);">
            <div class="flex items-center gap-2.5" style="padding: 10px; border-radius: 8px; background: rgba(255,255,255,0.04);">
                <div class="flex items-center justify-center" style="width: 34px; height: 34px; border-radius: 8px; background: var(--accent); color: var(--sidebar); font-weight: 700; font-size: 12px; flex: none;">
                    {{ strtoupper(substr($user?->name ?? 'BA', 0, 2)) }}
                </div>
                <div style="min-width: 0; flex: 1;">
                    <div class="truncate" style="font-size: 13px; font-weight: 600;">{{ $user?->branch?->name ?? 'Branch' }}</div>
                    <div class="truncate" style="font-size: 11px; opacity: 0.65;">{{ $user?->school?->name ?? 'School' }}</div>
                </div>
                <form method="POST" action="{{ route('v2.branch.logout') }}">
                    @csrf
    ...
    <main class="flex-1 flex flex-col" style="min-width: 0;">
        ...
        @if(session('success'))
            <div style="margin: 16px 28px 0; padding: 12px 16px; background: var(--ok-soft); border: 1px solid var(--ok); border-radius: 8px; font-size: 13px; color: var(--ok);">
                {{ session('success') }}
            </div>
        @endif
        @if(session('info'))
