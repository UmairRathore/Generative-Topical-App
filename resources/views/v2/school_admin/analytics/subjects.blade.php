...
@section('content')
<a href="{{ route('v2.school.analytics.index') }}" style="font-size: 13px; color: var(--text-soft); text-decoration: none; display: inline-flex; align-items: center; gap: 4px; margin-bottom: 16px;">
...
<p style="color: var(--text-soft); font-size: 13px; margin-bottom: 22px;">Performance by subject across your school.</p>

<div style="background: var(--surface); border: 1px solid var(--border); border-radius: var(--r-lg); overflow: hidden;">
    <table class="tbl" style="width: 100%; border-collapse: collapse;">
        <thead>
            <tr style="background: var(--soft-surface); border-bottom: 1px solid var(--border);">
                @foreach (['Subject', 'Classes', 'Students', 'Exams', 'Subs', 'Avg'] as $h)
        ...
        <tbody>
            @forelse ($rows as $r)
                <tr style="border-bottom: 1px solid var(--border); cursor: pointer;" onclick="window.location='{{ route('v2.school.analytics.subject', hid($r['id'])) }}'">
